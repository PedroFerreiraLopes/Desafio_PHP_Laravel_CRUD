...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="flex flex-col gap-2 h-screen ffont-sans antialiased dark:bg-black dark:text-white/50">
        <header class="flex gap-2 p-4 text-center top-0 w-full z-50"> {{-- class="grid grid-cols-2 items-center gap-2 py-10 lg:grid-cols-3" --}}
            @if (Route::has('login'))
                <nav class="-mx-3 flex flex-1 justify-end">
                    ...
                    @else
                        <a
                            href="{{ route('login') }}"
                            class="rounded-md px-3 py-2 text-black transition hover:text-black/70 focus:outline-none focus-visible:ring-[#FF2D20] dark:text-white dark:hover:text-white/80 dark:focus-visible:ring-white"
                        >
                            Entrar
                        </a>
